// src/app/(admin)/admin/schedules/new/NewScheduleClient.jsx
"use client";
...
export default function NewScheduleClient() {
  ...
  useEffect(() => {
    (async () => {
      const r = await fetch(
        "/api/public-courses?fields=_id,course_id,course_name&limit=1000"
      );
  ...
  async function handleSave() {
    ...
    if (isoDates.length === 0)
      return alert("กรุณาเลือกวันที่อบรมอย่างน้อย 1 วัน");
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="flex gap-3">
              {TYPE_OPTS.map((o) => (
                ...
                >
                  <span
                    className={`h-2.5 w-2.5 rounded-full ${o.dot}`}
                  />
                  {o.label}
                </button>
              ))}
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              ใช้เปิดแท็บใหม่เมื่อคลิกวันที่บนตาราง
            </p>
          </div>

          <div className="pt-2">
            <div className="text-sm opacity-70 mb-2">
              รวม {dates.length} วันอบรม
            </div>
            <button
              onClick={handleSave}
...
function CalendarPanel({ dates, setDates }) {
  ...
  const setToggle = (d) => {
    ...
    const has = dates.some(
      (x) => new Date(x).toDateString() === key
    );
    if (has)
      setDates(
        dates.filter(
...
function Month({ month, value = [], onToggle }) {
  ...
  const monthName = month.toLocaleString("th-TH", {
    month: "long",
    year: "numeric",
  });
  ...
  return (
    <div className="rounded-lg border border-white/10 p-3">
      <div className="mb-2 text-center font-medium text-slate-200">
        {monthName}
      </div>
