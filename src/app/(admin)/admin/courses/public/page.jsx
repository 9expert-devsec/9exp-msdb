// src/app/(admin)/admin/courses/public/page.jsx
"use client";
...
export default function PublicCoursesAdminPage() {
  ...
  const [loadingAll, setLoadingAll] = useState(true); // programs + skills
  const [loadingItems, setLoadingItems] = useState(true); // list items
  ...
  const fetchItems = async () => {
    ...
    let res;
    try {
      res = await fetch(`/api/public-courses?${qs.toString()}`, {
        cache: "no-store",
        signal: controller.signal,
      });
    } catch (e) {
      if (e?.name === "AbortError") return;
      console.error("Fetch /api/public-courses network error:", e);
      alert("โหลดรายการ Public Courses ไม่สำเร็จ (network)");
    ...
    try {
      ...
      // บางที API อาจตอบ html (redirect/login) ทำให้ parse ไม่ได้
      console.error("Fetch /api/public-courses json parse error:", e);
      alert("โหลดรายการ Public Courses ไม่สำเร็จ (invalid json)");
      setLoadingItems(false);
      return;
    }
    ...
    if (!res.ok || data?.ok === false) {
      ...
      alert(
        data?.error
          ...
          : `โหลดรายการ Public Courses ไม่สำเร็จ (${res.status})`,
      );
      setLoadingItems(false);
      return;
    }
                        ...
                        <div>
                          ...
                          {it.related_courses?.length > 0 && (
                            <div className="text-xs mt-1 opacity-60">
                              Related:{" "}
                              {it.related_courses
                                .map((r) => r.course_name || r.course_id || r)
                                .join(", ")}
                            </div>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={async () => {
                            ...
                            await fetch(
                              `/api/admin/public-courses?id=${it._id}`,
                              {
                                method: "DELETE",
                              },
                            );
